...
- 設定 > 記録・入力補助の「Map検索」がONの場合、記録の新規追加・編集・回追加の会場欄からApple Maps候補を検索できる。候補選択時は会場名・住所・座標をDraftへ反映し、保存時にVisitの座標とPlaceMasterの両方へ保存する。同名・同住所または同座標のPlaceMasterは再利用する。候補選択後に会場名を手入力で変更した場合は誤った位置を残さないため住所・座標の選択状態を解除する。設定OFFでは会場名の手入力だけを表示する。
