...
> **最終更新**: 2026-07-09（無料/有料境界表・最小マスター実装方針追加）
...
- `PersonMaster`: アーティスト、俳優、作家、画家、声優、演者などを横断で扱う人物マスター。Mystoriumの制作団体マスター同様、ジャンル別DBに分けず、タグ/役割で「アーティスト」「俳優」「作家」「監督」「演者」などを表現する。保存必須は表示名に留め、よみは任意だが検索/並び替え用に持てる。別名、メモ、公式URL、SNSはDB必須にせず、詳細オプション/アコーディオンで扱う。MusicBrainz ID / Wikidata QID / Apple Music IDなどの外部IDは、候補取得・名寄せ用のoptional内部IDとし、ユーザー入力必須にしない。類似名がある場合は保存時/検索時に「似た人物があります」と提示し、あとから重複統合できるUIを用意する。初期実装は最小モデルのみで、関係リンクUIは未実装。
- `PlaceMaster`: 会場/場所を横断で扱うマスター。劇場、ライブ会場、映画館、美術館、寺社、酒蔵、飲食店、テーマパーク/施設などを単一カテゴリで分断せず、タグ/種別で表現する。名称、よみ、住所、座標、公式URL、別名を持てる。名称+住所/座標/よみが近い場合は「同じ会場の可能性があります」と提示し、ユーザー確認で統合する。記録時は名称/住所/座標を `Visit` 側にもスナップショット保存し、PlaceMasterや参照DBが変わっても過去記録が壊れないようにする。初期実装は最小モデルのみで、Visit/参照DBとのリンクUIは未実装。
