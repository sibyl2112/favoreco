...
- `TicketOverviewView`: Calendar右上のチケットアイコンから開く全申込の横断一覧。`すべて / 要対応 / 検討・申込 / 取得済み / 終了` で絞り込み、申込総数・要対応数・取得済み数、状態、プレイガイド/入手経路、次のアクションを表示する。予定名、会場、主催、プレイガイド、名義/登録アカウント、状態、入手経路、メモを横断検索でき、右上の＋から新しい予定・チケットを追加できる。要対応は申込/発売開始、申込締切、当落発表、入金締切、発券開始の未来日に加え、状態未更新の申込締切超過・発売開始済み・当落確認・入金期限超過・発券可能を赤い要確認として残す。行から `PlanDetailView` へ遷移し、長押しから次の状態へクイック更新、左スワイプで申込編集、右スワイプと確認後に申込だけを非表示にできる。`TicketAttemptStatusUpdater` が予定詳細/申込編集/一覧で共通して保存・通知再予約/取消・アーカイブを処理する。
