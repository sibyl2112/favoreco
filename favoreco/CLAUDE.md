...
- `AddTicketPlanView`: 予定・チケット追加/編集フォーム。ジャンル、公演/イベント名、日時、開場、会場、公式URLを `Plan` として保存・更新し、必要に応じてチケットの「今の状態」（気になる / 抽選応募予定 / 発売待ち / 取得済み）を選ばせる。状態に応じて、抽選応募予定は申込開始/締切・当落発表・名義/FC/カード枠、発売待ちは発売開始・購入先、取得済みは入金/発券・金額・枚数・座席など必要項目だけを表示し、`TicketAttempt` として同時作成/更新する。名義/アカウントは `RegistrationIntegrationSettingsView` で登録した `TicketAccount` から選ぶ。保存後は通知設定とiOS通知許可に応じて `TicketNotificationScheduler` で通知再予約し、予約IDを `TicketAttempt.notificationSettingsRaw` に保持する。編集時に申込情報をOFFにした場合、既存の最新 `TicketAttempt` は削除せずアーカイブし、該当通知をキャンセルする。
- `EditTicketAttemptView`: `Plan` に紐づく申込単体の追加/編集フォーム。複数先行・複数名義を扱うため、`Plan` 全体とは独立して `TicketAttempt` を作成/更新できる。`AddTicketPlanView` と同じ「気になる / 抽選応募予定 / 発売待ち / 取得済み」の入口を持ち、選択に応じて区分、名義/アカウント、申込開始/締切、当落、入金、発券、金額、枚数、座席、購入URL、メモを出し分ける。保存後に通知を再予約する。既存申込の削除は物理削除ではなくアーカイブ＋通知キャンセル。
