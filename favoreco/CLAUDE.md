...
- `TicketOverviewView`: Calendar右上のチケットアイコンから開く全申込の横断一覧。`すべて / 要対応 / 検討・申込 / 取得済み / 終了` で絞り込み、申込総数・要対応数・取得済み数、状態、プレイガイド/入手経路、次のアクションを表示する。要対応は申込/発売開始、申込締切、当落発表、入金締切、発券開始の未来日がある申込を対象とし、行から `PlanDetailView` へ遷移する。
