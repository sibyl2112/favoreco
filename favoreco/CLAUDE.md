...
> **最終更新**: 2026-07-09（入力アコーディオン標準構成更新）
...
- 課金・プランは、無料 / Pro買い切り / Premiumサブスク / DBパック単体販売の4層で整理する。Pro買い切りは高額寄りで、ローカルのフル機能所有（自作カテゴリ、詳細統計、年間まとめ、エクスポート、上限緩和、買い切りテーマ/フォント等）を担う。PremiumサブスクはiCloud同期、自動バックアップ、継続更新される参照DBアクセス、月次/年次リキャップ、思い出再提示、限定テーマ/フォントなど継続価値を担う。価格感はMystoriumを参考に月額200〜250円、年額1,500円前後を仮説とする。DBパック単体販売は、特定カテゴリの参照DBを買い切りで使える選択肢としてPremiumと並べる。
- Mystoriumの同期実装後4プラン構造は参照モデルとして保持する。無料=¥0/ローカル制限あり/同期なし、ライト買い切り=¥1,500/ローカル全機能永久/同期なし、同期サブスク=月¥250または年¥1,500/ローカル全機能と同期を契約中のみ、フル買い切り=¥6,000/ローカル全機能永久/同期永久。フル¥6,000はライト¥1,500 + バックアップ・同期永久¥4,500の頭金方式で、どの購入ルートでも合計¥6,000に揃える。創設メンバー特典は、既存¥980ユーザー＋発売締切までの新規購入者に同期永久無料を付与する。favorecoではDBパック単体販売が加わるため、最終命名（Pro/Premium vs ライト/フル）とDBパックの関係は実装前に確定する。
