...
- `DataManagementView`: データ管理。保存データ件数、JSON/CSVエクスポート、JSON/CSVインポート、バックアップ説明、Webキャッシュ削除、写真サムネイルキャッシュ削除、アーカイブ済みデータの完全削除、全データ削除入口を持つ。キャッシュ削除では記録/写真本体を変更しない。アーカイブ完全削除は確認後に非表示の対象/予定/申込/マスターとその従属データを削除し、通知も取り消すが、非表示ジャンル設定は保持する。全データ削除の実処理は確認文言入力つき専用画面として次段階。JSONエクスポートは `JSONExportView` で復元前提の手動バックアップJSONを書き出し、CSVエクスポートは `CSVExportView` で全VisitをUTF-8 CSVとして手動書き出しできる。どちらも現時点では写真バイナリは含めない。
