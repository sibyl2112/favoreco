...
- `DisplaySettingsView`: 表示設定。Home各セクションのON/OFF、文字サイズ、将来の外観モード設定入口を持つ。文字サイズは初期状態でiOS設定に従い、任意でアプリ内の小さめ/標準/大きめ/特大を選べる。設定はContentViewのDynamic Type環境へ適用し、Noto Sans JP / Noto Serif JP / Cormorantを含む全画面へ反映する。
