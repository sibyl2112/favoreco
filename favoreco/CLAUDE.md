...
- `HomeView`: Hero直下に横断ミニ統計（今後の予定 / 今年の記録 / 総記録数）を表示し、その下にアテンション、体験ギャラリー、あとで記録、最近の記録、ジャンル一覧、統計サマリ、お気に入り/ベストの固定順で表示する。各セクションは設定でON/OFF可能。体験ギャラリー/最近の記録カードでは写真、日付、場所、人物・団体、チケット状態、評価、金額、OCR/詳細オプション有無、短いメモをサマリー表示する。カテゴリカードからカテゴリトップへ遷移し、右上プロフィールアイコンから設定へ遷移できる。
...
- `CalendarView`: Visitの日付を月表示で確認するアプリ内カレンダー。月移動、日付選択、記録がある日のドット表示、選択日の記録一覧、直近予定一覧を表示し、各記録から `ExperienceDetailView` へ遷移できる。
- `StatsView`: SwiftDataのVisit/RecordCategoryから、総記録数、今年の記録、今月の記録、平均評価、ジャンル別回数、記録済み金額を表示する簡易統計タブ。金額はプライバシー情報として初期表示では伏せ、目アイコンを押した時だけ表示する。下部に `月刊Favoreco` / `年間Favoreco` の思い出レポート予告枠を置き、基本統計=無料、詳細統計/年間まとめ=Pro、同期込み自動レポート=Premium候補という境界を画面上でも見せる。詳細統計、期間切替、年間まとめ生成、グラフ、予定/申込統計は未実装。
