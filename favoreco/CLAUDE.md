...
> **最終更新**: 2026-07-09（DBパック方針を検討扱いに更新）
...
- `PersonMaster`（未実装）: アーティスト、俳優、作家、画家、声優、演者などを横断で扱う人物マスター。Mystoriumの制作団体マスター同様、ジャンル別DBに分けず、タグ/役割で「アーティスト」「俳優」「作家」「監督」「演者」などを表現する。保存必須は表示名に留め、よみは任意だが検索/並び替え用に持てる。別名、メモ、公式URL、SNSはDB必須にせず、詳細オプション/アコーディオンで扱う。MusicBrainz ID / Wikidata QID / Apple Music IDなどの外部IDは、候補取得・名寄せ用のoptional内部IDとし、ユーザー入力必須にしない。類似名がある場合は保存時/検索時に「似た人物があります」と提示し、あとから重複統合できるUIを用意する。
...
- `AddExperienceView`: 最小記録追加フォーム。入力中は `AddExperienceDraft` に保持し、保存時だけ `ExperienceEvent` + `Visit` を作成する。カテゴリ別にフォーム文言を切り替え、公式URLも保存できる。将来の本入力UIでは、基本情報/人物・団体/チケット・予定/写真/OCR・取込/金額/公式情報/メモなどのユニット単位アコーディオンに移行する。
...
- `ExperienceDetailView`: 保存済みVisitの詳細表示。記録詳細では保存した情報を原則すべて見せる。Home/一覧/サマリーカードでは写真、タイトル、日付、ジャンル、場所、短いメモなど厳選した情報だけを表示し、詳細画面で写真、人物、チケット、場所、天気、金額、URL、SNS、メモなどへ深掘りできる。
