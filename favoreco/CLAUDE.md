...
> **最終更新**: 2026-07-09（人物/アーティストマスター方針追加）
...
- `PersonMaster`（未実装）: アーティスト、俳優、作家、画家、声優、演者などを横断で扱う人物マスター。保存必須は表示名と種別/役割に留め、よみは任意だが検索/並び替え用に持てる。別名、メモ、公式URL、SNSはDB必須にせず、詳細オプション/アコーディオンで扱う。MusicBrainz ID / Wikidata QID / Apple Music IDなどの外部IDは、候補取得・名寄せ用のoptional内部IDとし、ユーザー入力必須にしない。
...
- 人物/アーティストマスターは、巨大なタレント名鑑を自前保持する設計にしない。無料/ローカルではユーザーが記録した人物を再利用する個人用マスター、Premium/サブスクでは外部候補読み込み（MusicBrainz / Wikidata / Apple Music等）を入力補助としてONにする。外部候補は補助・キャッシュであり、記録の正本はユーザーが保存したスナップショットとする。別名、メモ、公式URL、SNSは必須項目にせず、詳細オプション/アコーディオンに置く。
