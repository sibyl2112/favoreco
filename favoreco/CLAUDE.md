...
- `RecordCategory`: ユーザー定義カテゴリ。テンプレも保存済みカテゴリとして扱う。初回起動時に `CategoryPresetSeeder` が標準カテゴリをfetch-first方式で注入/更新する。初回ジャンル選択後は `isArchived` で表示対象を制御し、少なくとも1件は有効になるよう補正する。
...
- `SettingsView`: 設定画面。プロフィール、初回ジャンル選択のやり直し、写真付き仮データ追加のデバッグボタンを持つ。
