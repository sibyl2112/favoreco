...
- `AddExperienceView`: 記録追加フォーム。入力中は `AddExperienceDraft` と写真用の一時状態に保持し、保存時だけ `ExperienceEvent` + `Visit` + `PhotoBlob` を作成する。`RecordCategory.enabledUnitsRaw` に応じてユニット単位アコーディオンを表示し、現時点では `basic`（対象名/日付/場所/評価）、`photos`（写真ライブラリから追加・10枚上限）、`officialInfo`（公式URL）、`memo`（メモ）を実入力、その他は準備中表示にする。
- `AddVisitView`: 既存 `ExperienceEvent` に新しい `Visit` だけを追加するフォーム。ユニット単位アコーディオンを表示し、`basic`、`photos`、`memo` を実入力、その他は準備中表示にする。
...
- `EditExperienceView`: 保存済み記録の編集フォーム。既存 `ExperienceEvent` + `Visit` を更新する。記録追加と同じユニット単位アコーディオンを使い、長いジャンル入力に備える。写真ユニットでは保存済み写真の確認/削除と新規写真追加ができる。
- `ExperienceDetailView`: 保存済みVisitの詳細表示。記録詳細では保存した情報を原則すべて見せる。Home/一覧/サマリーカードではカバー写真、ジャンル色/アイコン、タイトル、日付、場所、人物/団体1〜2件、評価またはステータス、短いメモ1行など厳選した情報だけを表示し、詳細画面で写真、人物、チケット、場所、天気、金額、URL、SNS、メモなどへ深掘りできる。`PhotoBlob.data` がある場合は1枚なら大きく、複数枚ならグリッドで表示する。
