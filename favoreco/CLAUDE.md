...
> **最終更新**: 2026-07-11（進捗監査・Home固定順の再整合）
...
- 記録・入力補助の初期値は、デフォルト記録日=今日、デフォルトジャンル=最後に使ったジャンル、記録追加後=詳細を開く、写真追加=カメラを開く、写真圧縮=85%、URL/OCR/Map/天気/入力補助辞書=ONとする。接続済みはデフォルトジャンル、記録追加後、写真追加初期動作、写真圧縮、URL候補、OCR取込、Map検索、入力補助辞書。写真追加は設定側を主要ボタンとして先に表示し、カメラ/写真ライブラリのもう一方も補助ボタンとして常に選べる。カメラ画像とライブラリ画像は同じ長辺1600px・選択品質・メタデータ非引継ぎ・1記録10枚上限・初回カバー設定を通す。デフォルトジャンルは「最後に保存成功したジャンル」または「Homeから開いた/ジャンル画面で選択中のジャンル」を別々に保持し、中央＋のジャンル候補で既定ジャンルを先頭に明示する。対象ジャンルが非表示なら先頭の有効ジャンルへフォールバックし、他ジャンルも同じダイアログから選べる。記録追加後は新規記録/回追加の保存成功後に保存済みVisit詳細を開き、詳細の「完了」で追加シートを閉じる。「一覧に戻る」選択時は保存後すぐ追加シートを閉じる。編集保存には適用しない。未接続は天気自動付与。
