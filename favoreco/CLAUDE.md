...
- `PlanDetailView`: 予定・チケット詳細。`Plan` のタイトル、ジャンル、日時、開場、会場、主催、公式URL/参考URL、メモを表示し、紐づく `TicketAttempt` の状態、先行区分、名義/アカウント、申込開始/締切、当落、入金、発券、金額、座席、購入URL、メモを確認できる。メニューから予定編集、申込追加、カレンダーに追加、予定削除ができる。各申込カードをタップすると `EditTicketAttemptView` で編集できる。予定削除は物理削除ではなく `Plan.isArchived` と申込アーカイブで非表示にし、予約済み通知をキャンセルする。
