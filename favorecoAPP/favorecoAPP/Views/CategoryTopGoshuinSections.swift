...
private struct GoshuinMapCoordinateRevision: Equatable {
    let visitID: UUID
...
struct GoshuinMapPreview: View {
    ...
    private var items: [GoshuinMapItem] {
        visits.compactMap { visit in
            guard let coordinate = Self.storedCoordinate(for: visit),
    ...
    private var coordinateRevision: [GoshuinMapCoordinateRevision] {
        visits.map { visit in
            let place = visit.placeMaster
            return GoshuinMapCoordinateRevision(
                visitID: visit.id,
                visitLatitude: visit.latitude,
                visitLongitude: visit.longitude,
                placeID: place?.id,
                placeLatitude: place?.latitude ?? 0,
                placeLongitude: place?.longitude ?? 0
            )
        }
    }
    ...
    var body: some View {
        ...
        .onChange(of: coordinateRevision, initial: true) { _, _ in
            selectedItemID = nil
