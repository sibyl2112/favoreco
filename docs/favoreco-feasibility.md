...
- **A: 体験特化カレンダーのGoogle式表示**
  ...
  - **終日・期間ものの描き分け（2026-07-05 モックで確定・Googleの「終日帯」積み上げを廃止）**:
    1. 期間の状態（販売期間・会期）＝**期間バー**（セル下部を走る高さ約11pxの連続バー・カテゴリ/種別色・**バー内に白抜きタイトル**を開始日と各週の左端に表示）。**バーの頭にアイコン**：販売期間＝チケットアイコン、会期などイベント期間＝カテゴリのアイコン（SF Symbol）。複数重なればバーを下に積む（最大2段＋あふれ「+n」）。グリッド・チップ領域を占有しない
