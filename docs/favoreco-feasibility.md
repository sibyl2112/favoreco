...
## 4-3. カレンダー連携の方針（2026-07-05 確定・投入時期は未定）

- **EventKit（Apple標準）一本でApple/Googleカレンダー両対応**: iOSの設定でGoogleアカウントを繋いだユーザーのGoogleカレンダーは標準カレンダーDBに同期されるため、EventKit読み取りで両方をfavorecoカレンダーに**薄く重ね表示**できる（仕事・私用の予定との突き合わせ）。OAuth・サーバー・Google API契約は不要、データは端末外に出ない
- 逆方向（体験予定→標準カレンダーへの書き出し）も同じEventKitで対応
- 権限: iOS 17以降のカレンダーアクセス許可フロー（読み取りはフルアクセス必要）
- カレンダー機能全体の範囲（体験特化A/連携B/汎用C）と投入時期（v1/v1.x/v2）は保留中の論点
