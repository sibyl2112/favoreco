...
### 決定ログ・要確認
- **Event/Visit境界（レビュー確定・記録消失分の復元）**: 会場・総合評価・座席・写真・成果・タグ・同行者・金額を Visit 側に置く。理由=ツアー公演/再飲で「1対象=1評価・1会場」前提が破綻するため。★取り違えがないかユーザー確認を要する（spec-A1 §9）
- **アイキャッチ二段構え（同上）**: Event=代表アイキャッチ、Visit=回別override、表示はVisit優先→Eventフォールバック
- **U9スペック表のCloudKit互換表現**: 別モデル化せず unitFieldsRaw の JSON 配列で持つ（②-A持ち越し事項への回答）

### 残課題
- spec-A1 §9 の3点をユーザー確認
- ②-A 続き: A3（ユニット別フィールド）/ A4（テンプレ別プリセット具体リスト）/ A5（集計・カレンダー・地図）/ A6（周辺）/ A7（登録フロー）
