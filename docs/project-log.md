...
- **参照DBの課金枠（A/B/C）＝要確定**: (A)サブスク込み／(B)カテゴリDBパック単体／(C)買い切り込み。推奨(A)+(B)。守る線=「記録は無料（Apple POIでどこでも可）、課金はDBの便利さのみ」。
- WeatherKit履歴の取得範囲2021/8/1〜は要出典確認済み（Apple Developer Forums）。それ以前・海外はアイコンなし。

### 残課題
- 参照DB課金枠(A/B/C)のユーザー確定 → feasibility・CLAUDE.md §7課金3層へ反映
- 施設マスター・印/寺社城港マスター・WeatherKit・場所マスター共通部品化をfeasibilityに追記
- CLAUDE.md §7 テンプレ一覧の更新（テーマパーク→おでかけ施設／御朱印追加）
