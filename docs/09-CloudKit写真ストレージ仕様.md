# CloudKit対応の写真・メディアストレージ仕様（MediaAsset / externalStorage 方式）

> 2026-07-05 作成。Mystorium V10 で実装・実機検証済み（4,808枚・1,201.9MB を移行、失敗0件）の方式を、favoreco が最初から採用するための仕様書。
> 2026-07-08 更新: favorecoでは写真だけでなく動画・サムネ・captionを扱うため、実装名は `MediaAsset` を推奨する。既存文書の `PhotoBlob` は写真のみの旧称として読み替える。
...
favorecoでは、写真/動画を将来iPad同期へ拡張できるよう、`PhotoBlob` ではなく **MediaAsset** として設計する。v1では動画本体を同期しない場合でも、メタデータ・サムネ・caption・同期状態を最初から持つ。
...
final class MediaAsset {
    var id: UUID = UUID()
    var mediaType: String = "photo"     // photo / video
    var role: String = "photo"          // photo / collection / eyecatch / label / memo
    var relativePath: String = ""
    var thumbnailPath: String = ""
    var sourceKind: String = "appCopy"  // appCopy / photosReference / cloudOnly
    ...
    var caption: String = ""
    var note: String = ""
    var syncState: String = "localOnly" // localOnly / pendingUpload / uploading / synced / cloudOnly / downloading / failed
    var isOriginalStoredLocally: Bool = true
...
- デフォルトは **Photos参照 + サムネ + メタ情報**。動画本体をアプリ内へ自動コピーしない。
- アプリ内コピーはユーザーが明示した時だけ。
- Cloud同期ONでも、動画本体のアップロードは別途「動画同期ON」や明示操作を要求する。
- iPad側ではメタ/サムネ/caption/durationを先に表示し、再生時に本体をダウンロードする。
- Photos参照だけの場合、元動画が写真アプリから削除されると再生できない。その状態はUIで表示する。

### 同期状態

```swift
enum MediaSyncState: String {
    case localOnly
    case pendingUpload
    case uploading
    case synced
...
|---|---|
| localOnly | この端末のみ |
| pendingUpload / uploading | 同期待ち / アップロード中 |
| synced | iCloud同期済み |
| cloudOnly | タップでダウンロード |
| failed | 再試行 |

## 2-旧. 写真のみの最小モデル：PhotoBlob
...
PhotoBlob最小形はMystorium由来の実証済み方式。favorecoではMediaAssetへ拡張するが、`@Attribute(.externalStorage)`・`byteCount`・fetch-first upsert等の基本方針は同じ。
...
UI・保存処理（画面側は保存方式を知らない）
   ...
   ▼
MediaStorage（API 凍結・relativePath / mediaID 形式も不変）
   │
   ├─ 写真・アイキャッチ・サムネ → MediaAssetStore（SwiftData / externalStorage）
   ├─ Photos参照動画 → localIdentifier + thumbnail + metadata
   └─ アプリ内保存/同期対象動画 → MediaAsset originalAsset（将来CloudKit Asset）
```
...
- **動画を自動コピーしない理由**：容量圧迫を避けるため。通常はPhotos参照＋サムネで十分。残したい動画/iPadで見たい動画だけ、ユーザー明示でアプリ内保存・Cloud同期対象にする
