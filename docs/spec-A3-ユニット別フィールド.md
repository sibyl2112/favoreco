...
## 5-2. リスト画像OCR取込（U4汎用・2026-07-07 ユーザー要望）

> U4「リスト」ユニットに**写真/画像からの一括起こし**パスを追加する。セトリ専用ではなく**U4を使う全種別で共通**（呼び名だけ種別で切替）。Live Rock「画像をスキャンしてセットリストを保存」の汎用版。外部ライブラリゼロ（Vision）。

### 仕組み（共通）
1. 📷写真 → **撮影補正**（傾き/歪み補正・白飛ばしくっきり・feasibility §4-2）で紙/スクリーンの歪みを整える。
2. **Vision `VNRecognizeTextRequest`** でテキスト認識（縦書き対応）。
3. **順序化**：認識行を boundingBox の**縦位置でソート**。番号プレフィックス（`1.`/`①`）や改行を項目区切りとして解釈し、順序付き項目に分解。
4. **プレビュー編集**：訂正・**並び替え**・行の結合/分割/削除（U4は元々並び替え可）。OCRは**入力補助のみ・確定はユーザー**（A7 §3原則）。
5. 格納＝U4 **unitFieldsRaw**（順序付きJSON配列・既存）。

### 種別ごとの呼び名（同一ユニット・OCR元）
| 種別 | U4呼び名 | OCR元の例 |
|---|---|---|
...
### 美術展での住み分け（U4 と U11 の共存）
- **U4 出品目録OCR**＝観たもののチェックリスト（作品名の羅列を目録から一括起こし）。
- **U11 コレクション**＝自分で撮った個別作品の写真＋キャプション（絵画の作品名・A3 §4 U11）。
- 両立。**曲名/作品名は名寄せ正規化**して A5 の演奏回数／再訪集計に接続（spec-A8 §5・A5 §1）。
